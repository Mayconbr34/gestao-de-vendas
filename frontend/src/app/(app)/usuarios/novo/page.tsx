...
const STEPS = [{ n: 1, label: 'Usuário', desc: 'Dados da conta' }];
...
export default function NovoUsuarioPage() {
  ...
  const canSave = email.trim() && password.trim();
    ...
    <div style={{ background: tk.bg, minHeight: '100%', padding: 0, fontFamily: 'var(--np-font)' }}>
      ...
      <div style={{ width: '100%', margin: 0, display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <div style={{ fontSize: '22px', fontWeight: '700', color: tk.text, letterSpacing: '-0.03em' }}>Novo usuário</div>
            <div style={{ fontSize: '13.5px', color: tk.textSub, marginTop: '4px' }}>Crie uma conta para a equipe.</div>
          </div>
          <button
        ...
        <div style={{ background: tk.surface, border: `1px solid ${tk.border}`, borderRadius: '12px', overflow: 'hidden', boxShadow: tk.shadow }}>
          <div style={{ padding: '20px 24px', borderBottom: `1px solid ${tk.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: '15px', fontWeight: '600', color: tk.text, letterSpacing: '-0.01em' }}>Dados da conta</div>
              <div style={{ fontSize: '12.5px', color: tk.textSub, marginTop: '2px' }}>Informe email, senha e permissões.</div>
            </div>
          ...
          <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              ...
              <Field label="Email *" tk={tk}>
                <Input value={email} onChange={setEmail} placeholder="email@empresa.com" tk={tk} />
              </Field>
            </div>
            <Field label="Senha *" tk={tk} hint="Defina uma senha temporária. O usuário poderá alterar depois.">
              <Input value={password} onChange={setPassword} placeholder="Senha" type="password" tk={tk} />
            </Field>
                ...
                {user?.role === 'SUPER_ADMIN' ? (
                  <Field label="Empresa" tk={tk} hint="Opcional para SUPER_ADMIN.">
                    <Select
                      ...
                      options={[{ value: '', label: 'Selecione' }, ...companies.map((c) => ({ value: c.id, label: c.tradeName }))]}
                      tk={tk}
                    />
                  </Field>
                ) : (
          ...
          <div style={{ padding: '16px 24px', borderTop: `1px solid ${tk.border}`, display: 'flex', justifyContent: 'flex-end' }}>
            <button
              ...
              style={{
                padding: '9px 16px',
                background: canSave ? tk.accent : tk.disabledBg,
                border: `1px solid ${canSave ? tk.accent : tk.border}`,
                color: canSave ? '#fff' : tk.textMuted,
                borderRadius: '8px',
